...
def divide_data():
    ...
    :return:
    '''
    with open('./data/log_data-3-class.json', encoding='utf8') as i_f:
        stus = json.load(i_f)
    # 1. delete students who have fewer than min_log response logs
    stu_i = 0
    ...
    for stu in stus:
        ...
        for log in stu['logs']:
            logs.append(log)
        random.shuffle(logs) #随机打乱学生的练习日志
    ...
    with open('./data/train_slice-3-class.json', 'w', encoding='utf8') as output_file:
        json.dump(train_slice, output_file, indent=4, ensure_ascii=False)
    with open('./data/train_set-3-class.json', 'w', encoding='utf8') as output_file:
        json.dump(train_set, output_file, indent=4, ensure_ascii=False)
    with open('./data/val_set-3-class.json', 'w', encoding='utf8') as output_file:
        json.dump(val_set, output_file, indent=4, ensure_ascii=False)    # 直接用test_set作为val_set
    with open('./data/test_set-3-class.json', 'w', encoding='utf8') as output_file:
        json.dump(test_set, output_file, indent=4, ensure_ascii=False)

    print("训练0.7-验证0.1-测试0.2分割保存完毕！")
